    ...
    $.get( "/api/match", { league: "NFL"} )
  		.done(function( data ) {
    		data.forEach(function(arrayItem){
    			...
    			$("#matches").append(`<p><strong>${arrayItem.match.visitorTeamName} @ ${arrayItem.match.homeTeamName}</strong>
    			...
    			<br>Your prediction - ${arrayItem.match.visitorTeamName}: <input size="2" type="text" name="visitorScorePrediction" value="${arrayItem.predictedVisitorScore != null ? arrayItem.predictedVisitorScore : ""}"> ${arrayItem.match.homeTeamName}: <input size="2" type="text" name="homeScorePrediction" value="${arrayItem.predictedHomeScore != null ? arrayItem.predictedHomeScore : ""}"></p>`);
    		});
  		}
  	);
});